...
function buildPlayer(id, team, role) {
  const tile = { x: team === "blue" ? BLUE_BASE_X + 4 : RED_BASE_X - 4, y: 0 };
  let player = createPlayer(id, team, tile);
...
let state = {
  tick: 0,
  blue: buildPlayer("blue-1", "blue", "melee"),
  red: buildPlayer("red-1", "red", "ranged"),
  minions: [],
  towers: [
    makeTower("blue-tower", "blue", BLUE_TOWER_X),
    makeTower("red-tower", "red", RED_TOWER_X)
  ],
  log: [],
  rng: Math.random,
  humanControl: { attackEnabled: true }
};
