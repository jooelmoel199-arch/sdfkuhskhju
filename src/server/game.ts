...
export interface Player {
  ...
  attackQueuedTick:number|null; hitQueuedTick:number|null; pendingHitDamage:number; pendingHitRoll:number; pendingDefenceRoll:number; pendingHitTargetId:string|null; pendingSpecial:boolean; specialQueued:boolean; path:Tile[];
}
...
function makePlayer(id:string,name:string,team:Team,x:number,y:number):Player{
  return {id,name,team,x,y,destinationX:x,destinationY:y,hp:99,maxHp:99,prayerPoints:20,maxPrayerPoints:20,
    ...
    inventory:{slots:[{id:"rune_scimitar",quantity:1},{id:"lobster",quantity:10},{id:"coins",quantity:2500},null,null,null,null,null,null,null,null,null],food:10,specialEnergy:100,coins:2500},prayer:null,attackStyle:"accurate",targetId:null,nextAttackTick:0,attackQueuedTick:null,hitQueuedTick:null,pendingHitDamage:0,pendingHitRoll:0,pendingDefenceRoll:0,pendingHitTargetId:null,pendingSpecial:false,specialQueued:false,path:[]};
}
...
function inMeleeRange(a:Player,b:Player):boolean{return Math.abs(a.x-b.x)+Math.abs(a.y-b.y)===1;}
...
function resolveAttack(state:GameState,a:Player):void{
 ...
 const d=state.players[a.targetId];if(!d||d.hp<=0){a.targetId=null;a.attackQueuedTick=null;a.hitQueuedTick=null;a.pendingHitDamage=0;a.pendingHitTargetId=null;a.pendingSpecial=false;a.specialQueued=false;return;}
