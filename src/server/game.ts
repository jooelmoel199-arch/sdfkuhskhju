...
export interface Equipment {
  weapon: string; attackType: AttackType; attackRange: number; attackSpeed: number; attackBonus: number; strengthBonus: number; magicAttackBonus?: number; magicDamageBonus?: number; rangedStrengthBonus?: number; prayerBonus?: number;
  specialCost: number; specialMultiplier: number; defenceBonus: number; defenceRanged:number; defenceMagic:number; defenceStab: number; defenceSlash: number; defenceCrush: number;
...
function makePlayer(pid:number,id:string,name:string,team:Team,x:number,y:number):Player{
  return {pid,id,name,team,x,y,destinationX:x,destinationY:y,hp:99,maxHp:99,prayerPoints:20,maxPrayerPoints:20,
    attack:75,strength:75,defence:70,ranged:75,magic:75,xp:{attack:0,strength:0,defence:0,ranged:0,magic:0,hitpoints:0},equipment:{weapon:WEAPONS.rune_scimitar.id,...WEAPONS.rune_scimitar, defenceBonus:0, defenceRanged:0, defenceMagic:0, defenceStab:0, defenceSlash:0, defenceCrush:0},
...
function processInput(state:GameState,command:InputCommand):void{
 ...
 switch(command.type){
  ...
  case "item_action":{const stack=p.inventory.slots[command.slot];if(!stack||stack.quantity<=0)return;if(command.action==="eat"&&stack.id==="lobster"&&p.hp<p.maxHp){stack.quantity--;p.inventory.food=Math.max(0,p.inventory.food-1);p.hp=Math.min(p.maxHp,p.hp+12);event(state,{tick:state.tick,type:"eat",attacker:p.id,damage:-12});if(stack.quantity===0)p.inventory.slots[command.slot]=null;return;}if(command.action==="equip"){
      if(stack.id==="rune_scimitar"){Object.assign(p.equipment,{...WEAPONS.rune_scimitar,defenceBonus:0,defenceRanged:0,defenceMagic:0,defenceStab:0,defenceSlash:0,defenceCrush:0});delete p.equipment.ammoId;delete p.equipment.spellId;event(state,{tick:state.tick,type:"attack_style",attacker:p.id,reason:"equipped rune scimitar"});return;}
      if(stack.id==="shortbow"){Object.assign(p.equipment,{...WEAPONS.shortbow,defenceBonus:0,defenceRanged:0,defenceMagic:0,defenceStab:0,defenceSlash:0,defenceCrush:0,ammoId:"bronze_arrow"});delete p.equipment.spellId;event(state,{tick:state.tick,type:"attack_style",attacker:p.id,reason:"equipped shortbow"});return;}
      if(stack.id==="fire_strike"||stack.id==="ice_barrage"){const spell=SPELLS[stack.id];Object.assign(p.equipment,{...WEAPONS.fire_strike,attackSpeed:spell.attackSpeed,attackRange:spell.attackRange,magicAttackBonus:spell.magicAttackBonus,defenceBonus:0,defenceRanged:0,defenceMagic:0,defenceStab:0,defenceSlash:0,defenceCrush:0,spellId:spell.id});delete p.equipment.ammoId;event(state,{tick:state.tick,type:"attack_style",attacker:p.id,reason:"selected "+stack.id});return;}
      return;
    }return;}
...
function resolveRangedOrMagicAttack(state:GameState,a:Player,d:Player):void{
 ...
 const attackBonus=ranged?a.equipment.attackBonus+(AMMUNITION[a.equipment.ammoId??""]?.attackBonus??0):(a.equipment.magicAttackBonus??0), defenceBonus=ranged?d.equipment.defenceRanged:d.equipment.defenceMagic;
 ...
 const maxHit=ranged?standardMaxHit(effectiveRangedStrength,(a.equipment.rangedStrengthBonus??0)+(ammo?.rangedStrength??0)):magicMaxHit(spell?.maxHit??0,(a.equipment.magicDamageBonus??0)+attackerPrayer.magicDamage);
