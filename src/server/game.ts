...
function processInput(state:GameState,command:InputCommand):void{
 ...
 switch(command.type){
  ...
  case "prayer":if(command.prayer!==null&&p.prayerPoints<=0)return;p.prayer=command.prayer;p.prayerNextDrainTick=command.prayer===null?null:state.tick+1;if(command.prayer!==null)p.prayerDrainCounter=0;event(state,{tick:state.tick,type:"prayer",attacker:p.id,prayer:p.prayer});return;
