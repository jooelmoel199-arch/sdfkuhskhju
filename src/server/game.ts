...
export interface Player {
  ...
  attackQueuedTick:number|null; hitQueuedTick:number|null; pendingHitDamage:number; pendingHitRoll:number; pendingDefenceRoll:number; pendingHitTargetId:string|null; pendingAttackType:"melee"|null; pendingSpecial:boolean; specialQueued:boolean; path:Tile[];
}
...
function resolveAttack(state:GameState,a:Player):void{
 ...
 a.pendingHitDamage=damage;a.pendingHitRoll=attackRoll;a.pendingDefenceRoll=defenceRoll;a.pendingHitTargetId=d.id;a.pendingAttackType="melee";a.pendingSpecial=special;
}
...
function resolveQueuedHitForPlayer(state:GameState,p:Player):void{
 ...
 for(const a of incoming){
   ...
   if(p.hp<=0)continue;
   const damage=(rawDamage>0&&a.pendingAttackType==="melee"&&p.prayer==="protect_melee")?Math.floor(rawDamage*0.6):rawDamage;
